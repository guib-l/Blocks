...
# Run a single node (inline JSON)
blocks node run my_node -i '{"n": 10}'

# Run a single node (JSON file)
blocks node run my_node -i data.json
...
# Run a workflow with input data (inline JSON)
blocks workflow run my_workflow -i '{"x": 5}'

# Run a workflow with input data (JSON file)
blocks workflow run my_workflow -i data.json
